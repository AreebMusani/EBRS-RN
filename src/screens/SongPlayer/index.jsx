...
import {getSongsDataById, updateSongData} from '../../redux/slices/songs';
...
const SongPlayer = ({navigation, route, showAlert}) => {
  ...
  const {_id, key, artwork, title, Category, Emotion, Rating, Ratings, artist, liked} =
    selectedSong || {};
  // ref
  const bottomSheetRef = useRef(null);
  const snapPoints = useMemo(() => [hp('23%'), hp('50%')], []);

  // const getSongData = useSelector(getSongsDataById);
  const state = useSelector(state => state);
  // let iddddd = route?.params?.data?._id;

  // const data = state.song.listOfSongs.find(song => song?._id === _id);

  const getSongById = useCallback(
    async id => {
      const data = state?.song?.listOfSongs.find(song => song?._id === id);
      console.log(data?.liked);

      setselectedSong(data);
      return data;
    },
    [route?.params?.data?._id, state?.song?.listOfSongs],
  );

  useEffect(() => {
    const findSong = getSongById(route?.params?.data?._id);
    console.log('findSong', findSong);
    // setselectedSong(findSong);
  }, [route?.params?.data?._id, state?.song?.listOfSongs]);

  // console.log("getSongs ", state?.song?.listOfSongs.find(song => song?._id === _id));
  ...
  useEffect(() => {
    // if(selectedSong?.liked){
    //   setisSongLiked(true);
    // }
    // console.log(Object.keys(selectedSong).toString());
  }, [selectedSong]);
  ...
  useEffect(() => {
    TrackPlayer.addEventListener(Event.PlaybackActiveTrackChanged, data => {
      ...
      getSongById(data?.track?._id);
      // console.log(data?.track?._id, " track");
      // iddddd = data?.track?._id;
    });
  });
  ...
  const giveSongRating = async () => {
    ...
    try {
      ...
      console.log(payload);
      const response = await api.giveRatingToSong({
        body: payload?.body,
  ...
  const onLikeSong = async () => {
    try {
      ...
      const response = selectedSong?.liked
        ? await api.removeLikedSong(data)
        : await api.addFavouriteSong(data);
      // console.warn({...selectedSong, liked: !selectedSong?.liked});
      ...
      dispatch(
        updateSongData({
          id: _id,
          data: {...selectedSong, liked: !selectedSong?.liked},
        }),
      );
      ...
      <View style={{flex: 1, justifyContent: 'center'}}>
        ...
        <View
          ...
          <View style={{flexShrink: 1}}>
            ...
            <Text numberOfLines={1} style={styles.songDetails}>Artist: {artist}</Text>
          </View>
        ...
        <PlayerProgressBar
          style={{width: '100%', paddingHorizontal: 20, marginTop: hp('4%')}}
        />
        ...
        <View style={styles.controllerContainer}>
          ...
          <TouchableOpacity onPress={toggleButton} style={styles.songToggle}>
            <FontAwesome
          ...
          <TouchableOpacity
            ...
            onPress={() => onLikeSong()}>
            <FontAwesome
              name={selectedSong?.liked ? 'heart' : 'heart-o'}
              color={colors.TEXT}
              size={hp('3%')}
            />
          </TouchableOpacity>
